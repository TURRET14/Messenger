...
async def update_user(
    data: backend.models.pydantic_request_models.UserUpdateModel,
...
async def update_user_login(
    data: backend.models.pydantic_request_models.UserUpdateLoginModel,
    selected_user: backend.storage.database.User,
    db: sqlalchemy.orm.session.Session) -> fastapi.responses.JSONResponse:
...
async def update_user_password(
    data: backend.models.pydantic_request_models.UserUpdatePasswordModel,
    selected_user: backend.storage.database.User,
    db: sqlalchemy.orm.session.Session) -> fastapi.responses.JSONResponse:
...
async def get_users_by_names(
    ...
    if name is None and surname is None and second_name is None:
        raise fastapi.exceptions.HTTPException(detail = "NO_ARGUMENTS_PROVIDED_ERROR", status_code = fastapi.status.HTTP_400_BAD_REQUEST)
    ...
    select_request = (select_request.order_by(backend.storage.database.User.id)
...
async def get_user_friends(
    ...
    friends_list = db.execute((sqlalchemy.select(
    backend.storage.database.User.id,
    backend.storage.database.User.username,
    backend.storage.database.User.name,
    backend.storage.database.User.surname,
    ...
    return fastapi.responses.JSONResponse(friends_list, status_code = fastapi.status.HTTP_200_OK)
...
async def get_user_sent_friend_requests(
    ...
    friend_requests_list = db.execute(sqlalchemy.select(
    backend.storage.database.User.id,
    backend.storage.database.User.username,
    backend.storage.database.User.name,
    backend.storage.database.User.surname,
    ...
    return fastapi.responses.JSONResponse(friend_requests_list, status_code = fastapi.status.HTTP_200_OK)
...
async def accept_friend_request(
    ...
    user_friend: backend.storage.database.UserFriend = backend.storage.database.UserFriend(
