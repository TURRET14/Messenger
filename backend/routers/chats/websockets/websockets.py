...
async def memberships_websocket_connection_body(
    ...
    await connection_manager.add_websocket(websocket, selected_user_id, websocket_type, selected_chat_id)

    try:
        while True:
            await websocket.receive_text()
    except WebSocketException:
        await connection_manager.remove_websocket(websocket, selected_user_id, websocket_type, selected_chat_id)
...
async def websocket_chat_memberships_post(
    ...
    await memberships_websocket_connection_body(websocket, WebsocketType.CHAT_MEMBERSHIP_POST, selected_chat.id, selected_user.id, connection_manager)
...
async def websocket_chat_memberships_put(
    ...
    await memberships_websocket_connection_body(websocket, WebsocketType.CHAT_MEMBERSHIP_PUT, selected_chat.id, selected_user.id, connection_manager)
...
async def websocket_chat_memberships_delete(
    ...
    await memberships_websocket_connection_body(websocket, WebsocketType.CHAT_MEMBERSHIP_DELETE, selected_chat.id, selected_user.id, connection_manager)