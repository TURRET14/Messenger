...
async def websocket_messages_post_listener(
    ...
    while True:
        try:
            pubsub_subscription: redis.asyncio.client.PubSub = await redis_client.pubsub_subscribe(RedisPubsubChannel.MESSAGES_POST)
            async for selected_message_data in pubsub_subscription.listen():
                ...
                async with async_session_maker() as db:
                    await connection_manager.messages_post_update(selected_message_data_model, True, db)
        except Exception:
            logger.exception("Messages post listener failed")
...
async def websocket_messages_put_listener(
    ...
    while True:
        try:
            pubsub_subscription: redis.asyncio.client.PubSub = await redis_client.pubsub_subscribe(RedisPubsubChannel.MESSAGES_PUT)
            async for selected_message_data in pubsub_subscription.listen():
                if selected_message_data["type"] != "message":
                    continue
                ...
                selected_message_data_model: MessagePubsubWebsocketModel = MessagePubsubWebsocketModel.model_validate_json(selected_message_data)
                async with async_session_maker() as db:
                    await connection_manager.messages_post_update(selected_message_data_model, False, db)
        except Exception:
            logger.exception("Messages put listener failed")
            await asyncio.sleep(1)
    ...
    while True:
        try:
            ...
            async for selected_message_data in pubsub_subscription.listen():
                ...
                selected_message_data_model: MessagePubsubWebsocketModel = MessagePubsubWebsocketModel.model_validate_json(selected_message_data)
                async with async_session_maker() as db:
                    await connection_manager.messages_delete(selected_message_data_model, db)
        except Exception:
            logger.exception("Messages delete listener failed")
            await asyncio.sleep(1)
    ...
    while True:
        try:
            ...
            async for selected_message_read_mark_data in pubsub_subscription.listen():
                ...
                await connection_manager.message_read_mark_post(selected_message_read_mark_data_model)
        except Exception:
            logger.exception("Messages read listener failed")
            await asyncio.sleep(1)
